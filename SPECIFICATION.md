...
- **Y軸**: 下方向が正 (Y-Down)。
- **回転**:
  - **Visual**: キャンバス上では各キーを**幾何中心**を軸に回転させる (Konva の Group rotation)。`rotationCenter` フィールドはキャンバス描画には使用しない。
  - **QMK**: 絶対座標としての `rx`, `ry` が必要。`rotationCenter` はこの計算時のオフセットとして使用する (通常 `{0, 0}`)。
  - **KiCad**: コンポーネントはその中心に配置され、挿入点を中心に回転。
- **備考**: `KeyData.rotationCenter` は現在 `{0, 0}` 固定で扱われており、編集 UI は存在しない。将来的に非中心回転をサポートする場合に備えたフィールドである。

## 5. 機能とアルゴリズム

### マトリクス自動割り当て (Auto-Assign)
物理的な配置に基づいて、電気的な行(Row)/列(Col)を自動的に割り当てます。
1. **ソート**: キーをY座標の量子化バケット (0.1U 単位) で比較してソートし、同一バケット内ではX座標順に並べます。この量子化により、比較関数の推移律が保証されソート結果が安定します。
2. **割り当て**: ソートされたキー順に列インデックスをインクリメントし、Y座標のズレが 0.1U を超えたタイミングで行インデックスを更新して列をリセットします。
3. **開始インデックス指定**: UI から Row / Col の開始インデックスを指定できます (デフォルト 0)。全キーまたは選択キーのみを対象として実行可能です。
...
### 一括キー追加
TopBar の "Count" フィールドで個数を指定し、複数のキーを一括で追加できます。キーはX方向に隣接して配置されます。

### キーナビゲーション (Tab)
RightSidebar でキーが選択されている状態で Tab / Shift+Tab を押すと、キーをY→X順に巡回して選択できます。

### 複数選択の一括回転
複数キーを選択した状態で回転ハンドルをドラッグすると、ドラッグしたキーを軸として選択中の全キーがオービット回転します。

### パン操作
- **Space + ドラッグ** または **中ボタンドラッグ**: キャンバスをパン。
- **ピンチジェスチャ** (タッチデバイス): 2本指でパン・ズームを同時制御。

### プリセットからのキー追加
左サイドバーのプリセットをクリックするとキャンバス中央に追加されます。既存キーと重なる場合はグリッド幅単位で右方向に自動回避します (最大100回試行)。ドラッグ＆ドロップでも追加可能です。

### データマイグレーション
旧フォーマットのプロジェクトデータを自動マイグレーションします:
- `visualLegend` → `legends.top`
- `legends.tl/tr/bl/br` → `legends.top/right/left/bottom`

### ストア永続化の範囲
`gridSize` / `snapEnabled` / `scale` / `pan` は永続化対象外です。リロード時にデフォルト値に戻ります。永続化されるのは `project` と `savedProjects` のみです。
